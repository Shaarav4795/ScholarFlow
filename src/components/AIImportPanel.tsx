...
export function AIImportPanel(props: AIImportPanelProps) {
  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <div className="rounded-[2rem] border border-ink-200/70 bg-white/75 p-5 dark:border-ink-700 dark:bg-ink-900">
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-ink-500 dark:text-ink-300">Upload a brief and review the extracted tasks.</p>
          </div>
        </div>

        <label className="mb-4 flex cursor-pointer flex-col items-center justify-center rounded-[1.75rem] border border-dashed border-ink-300/80 bg-ink-50/80 px-5 py-8 text-center transition hover:border-coral-500 hover:bg-coral-500/6 dark:border-ink-700 dark:bg-ink-800">
          ...
          <span className="mt-2 text-xs text-ink-500 dark:text-ink-300">
        ...
        <label className="block space-y-2">
          ...
          <textarea
            ...
            rows={5}
            className="w-full rounded-2xl border border-ink-200/80 bg-white/80 px-4 py-3 text-sm text-ink-700 outline-none transition focus:border-sage-500 focus:ring-2 focus:ring-sage-500/20 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-50"
        ...
        <div className="mt-5 rounded-2xl border border-sage-500/20 bg-sage-500/10 p-4 text-sm text-ink-600 dark:text-ink-300">
          <div className="mb-2 font-semibold text-ink-700 dark:text-ink-100">Tips</div>
      ...
      <div className="rounded-[2rem] border border-ink-200/70 bg-white/75 p-5 dark:border-ink-700 dark:bg-ink-900">
        ...
        {summary ? (
          <div className="mb-4 rounded-2xl border border-sky-400/25 bg-sky-400/10 p-4 text-sm text-ink-600 dark:text-ink-300">
            {summary}
          </div>
        ) : null}

        {results.length === 0 ? (
          <div className="flex min-h-72 flex-col items-center justify-center rounded-[1.75rem] border border-dashed border-ink-300/80 bg-ink-50/65 p-6 text-center dark:border-ink-700 dark:bg-ink-800">
            <FileText className="mb-4 h-10 w-10 text-ink-400 dark:text-ink-300" />
            <h4 className="font-display text-2xl text-ink-700 dark:text-ink-100">No preview yet</h4>
          ...
          <div className="space-y-4">
            {results.map((assignment, index) => (
              <article
                ...
                className="rounded-[1.75rem] border border-ink-200/70 bg-ink-50/70 p-5 dark:border-ink-700 dark:bg-ink-800"
              >
                <div className="mb-3 flex flex-wrap items-center gap-3">
                  <span className="inline-flex rounded-full bg-ink-800 px-3 py-1 text-xs font-semibold text-white dark:bg-ink-100 dark:text-ink-900">
                    {assignment.subject}
                  </span>
                  <span
                ...
                <div className="mt-4 grid gap-2 md:grid-cols-2">
                  {(assignment.checkpoints ?? []).map((checkpoint) => (
                    <div
                      ...
                      className="rounded-2xl border border-ink-200/70 bg-white/85 px-4 py-3 text-sm dark:border-ink-700 dark:bg-ink-800"
                    >
                      <div className="mb-1 flex items-center gap-2 font-semibold text-ink-700 dark:text-ink-100">
                        <FileImage className="h-4 w-4 text-ink-400 dark:text-ink-300" />
                        {checkpoint.title}
                      </div>
