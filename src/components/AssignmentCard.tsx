...
export function AssignmentCard(props: AssignmentCardProps) {
  ...
  return (
    <article className="rounded-[1.6rem] border border-ink-200/70 bg-white/80 p-5 shadow-sm dark:border-white/10 dark:bg-white/6">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs uppercase tracking-[0.24em] text-ink-400 dark:text-ink-200/50">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            className="rounded-full border border-ink-200/80 bg-white/80 px-3 py-2 text-sm font-semibold text-ink-700 outline-none transition hover:border-ink-400 focus:border-sage-500 dark:border-white/10 dark:bg-ink-900/80 dark:text-ink-50"
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
        <div className="rounded-[1.25rem] border border-ink-200/70 bg-ink-50/80 p-4 dark:border-white/10 dark:bg-ink-900/40">
          <div className="mb-3 flex items-center justify-between gap-4">
            ...
            <div className="text-sm font-semibold text-ink-500 dark:text-ink-200/70">
              {progress}% done
            </div>
          </div>

          <div className="mb-4 h-3 overflow-hidden rounded-full bg-ink-200/80 dark:bg-white/10">
            <div
              className={cn(
          ...
          <div className="markdown-body text-sm leading-6 text-ink-600 dark:text-ink-100">
            <ReactMarkdown>{props.assignment.description}</ReactMarkdown>
          </div>
        </div>

        <div className="rounded-[1.25rem] border border-ink-200/70 bg-white/75 p-4 dark:border-white/10 dark:bg-white/6">
          <div className="mb-3">
            <p className="text-xs uppercase tracking-[0.24em] text-ink-400 dark:text-ink-200/50">
              Checklist
            </p>
          ...
          <div className="space-y-3">
            {props.assignment.checkpoints.map((checkpoint) => (
              <label
                key={checkpoint.id}
                className="flex items-start gap-3 rounded-2xl border border-ink-200/70 bg-ink-50/70 px-4 py-3 dark:border-white/10 dark:bg-ink-900/45"
              >
                <input
                  type="checkbox"
                ...
                <span className="min-w-0 flex-1">
                  <span
                    className={cn(
                      'block text-sm font-semibold text-ink-700 dark:text-ink-50',
                      checkpoint.completed && 'line-through opacity-60',
                    )}
                  >
                    {checkpoint.title}
                  </span>
                  <span className="mt-1 block text-xs text-ink-500 dark:text-ink-200/70">
